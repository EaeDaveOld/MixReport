...
def ApplyFilter():
    ...
    FILTER_BUTTON = DRIVER.find_element(
        By.XPATH, '//*[@id="master-vbtn-optionsdialogopenbutton"]')
    FILTER_BUTTON.click()

    # Click on stock filter
    SELECT_STOCK_FILTER = DRIVER.find_element(
        By.XPATH, '//option[@value="ESTOQUE_DISPONIVEL"]')
    ...
    # Send bigger then 0
    VALUE_FROM_STOCK = DRIVER.find_element(
        By.XPATH, '//input[@placeholder="Valor"]')
    VALUE_FROM_STOCK.clear()
    VALUE_FROM_STOCK.send_keys('0')

    # Click on supplier filter
    SELECT_SUPPLIER_FILTER = DRIVER.find_element(
        By.XPATH, '//option[@value="T3_NOME"]')
    SELECT_SUPPLIER_FILTER.click()

    # Send supplier code to supplier filter
    VALUE_FROM_SUPPLIER = DRIVER.find_element(
        By.XPATH, '(//input[@placeholder="Valor"])[2]')
    VALUE_FROM_SUPPLIER.clear()
    VALUE_FROM_SUPPLIER.send_keys(supplier_code)

    # Click on apply filter button
    APPLY_FILTER_BUTTON = DRIVER.find_element(
        By.XPATH, '//a[@class="fwrub btn btn-primary btnApply"]')
